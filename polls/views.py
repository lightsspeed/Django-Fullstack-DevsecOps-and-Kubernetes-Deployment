...
class PollListView(ListView):
    ...
    def get_queryset(self):
        return Poll.objects.filter(is_active=True, choices__isnull=False).distinct().order_by('-created_at')
    ...
    def form_valid(self, form):
        ...
        if form.is_valid() and choice_formset.is_valid():
            ...
            if not choice_formset.is_valid():
                messages.error(self.request, "There were errors in the poll choices. Ensure you have at least 2 choices.")
...
class VoteView(View):
    def post(self, request, slug):
        ...
        if user:
            ...
            if not poll.allow_multiple_votes and Vote.objects.filter(poll=poll, user=user).exists():
            ...
            if not poll.allow_multiple_votes and Vote.objects.filter(poll=poll, user__isnull=True, ip_address=ip).exists():
                messages.error(request, "Someone from your IP has already voted anonymously.")
                return redirect('polls:poll_results', slug=slug)
